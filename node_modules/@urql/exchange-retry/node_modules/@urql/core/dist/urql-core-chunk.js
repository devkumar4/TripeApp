...
async function* parseMultipartMixed(contentType, response) {
  ...
  var isPreamble = true;
  var payload;
  for await (var chunk of split(streamBody(response), boundary)) {
    if (isPreamble) {
      isPreamble = false;
    } else {
      try {
...
function makeFetchSource(operation, url, fetchOptions) {
  return wonka.filter(result => !!result)(wonka.fromAsyncIterable(fetchOperation(operation, url, fetchOptions)));
}
